    ...
    <div class="max-w-7xl mx-auto relative z-10">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-12 md:gap-24">
        <div v-for="(t, index) in testimonials" :key="index" 
             class="space-y-8 md:space-y-10 testimonial-card-yoga opacity-0 translate-y-10"
        >
          <div class="w-14 h-14 md:w-16 md:h-16 rounded-full border border-white/10 flex items-center justify-center text-accent">
            <svg viewBox="0 0 24 24" class="w-6 h-6 md:w-8 md:h-8 fill-current">
          ...
          <p class="text-2xl md:text-4xl font-serif italic leading-tight font-light">{{ t.quote }}</p>
          <div class="flex items-center gap-4 md:gap-6">
            <div class="w-10 h-10 md:w-12 md:h-12 rounded-full overflow-hidden grayscale border border-white/10">
              <img :src="'https://i.pravatar.cc/100?u=' + t.author" alt="avatar">
            </div>
            <div>
              <div class="font-bold uppercase tracking-[0.2em] text-[9px] md:text-[10px] text-accent">{{ t.author }}</div>
              <div class="text-white/30 text-[8px] md:text-[9px] uppercase tracking-widest mt-1">{{ t.role }}</div>
            </div>
          </div>
        </div>
      </div>
