<template>
  <section class="py-24 md:py-40 bg-primary text-white relative overflow-hidden px-6">
    <!-- Ethereal Background -->
    ...
    <div class="max-w-4xl mx-auto relative z-10 text-center space-y-12 md:space-y-16">
      <div class="flex flex-col items-center gap-6 md:gap-8">
        <div class="w-[1px] h-16 md:h-24 bg-gradient-to-b from-transparent via-accent to-transparent about-line opacity-0"></div>
        <h2 class="text-2xl sm:text-4xl md:text-6xl font-serif italic font-light about-h2 opacity-0 leading-tight">
          {{ title }}
        </h2>
      </div>
      
      <p class="text-xl md:text-3xl text-white/40 font-light leading-relaxed about-p opacity-0">
        {{ content }}
      </p>
...
onMounted(() => {
  ...
  tl.to('.about-line', { opacity: 1, scaleY: 1.5, duration: 1.5 })
    .to('.about-h2', { opacity: 1, y: -20, duration: 1.2 }, '-=1')
    .to('.about-p', { opacity: 1, y: -10, duration: 1.2 }, '-=0.8')
