<template>
  <section ref="section" class="bg-black overflow-hidden">
...
onMounted(() => {
  setTimeout(() => {
    ctx = gsap.context(() => {
      if (scrollContainer.value && section.value && window.innerWidth >= 768) {
...
onUnmounted(() => {
  // Common fix for pinned sections: refresh ScrollTrigger after unmounting 
  // to reset page height and recalculate positions of remaining triggers.
  setTimeout(() => {
    ScrollTrigger.refresh();
  }, 50);
});
